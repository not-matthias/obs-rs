...
impl Capture {
    ...
    /// - Frame
    /// - Width and Height
    pub fn capture_frame<T: Clone>(&mut self) -> Result<(Vec<T>, (usize, usize)), ObsError> {
        ...
        let mut pixel_buf = Vec::with_capacity(byte_size(width * height));
        let mapped_pixels =
